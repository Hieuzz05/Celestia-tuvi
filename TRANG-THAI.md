...
| CEL-144 bộ đo ĐỘ ĐÚNG: `scripts/do-nhan-dien.ts --chi-bai-luan` (nhận diện lá số, có đáp án — dùng làm thước "bài có riêng cho lá số không" cho mọi thay đổi prompt) + `scripts/do-do-dung.ts` (xếp loại nhận định — giám khảo máy quá dễ dãi, đừng tin một mình). Đo lại lát cắt Sự nghiệp: thư viện KHÔNG làm bài đúng hơn → vẫn tắt (KIEN-TRUC 11.5–11.6). Chỉ chạy script, không đổi hành vi production | `viec/do-do-dung` | 26/09/2026 |
| **ĐÃ GỘP, THƯ VIỆN CHƯA BẬT**: CEL-143 `KIEN-TRUC-LUAN-GIAI.md` v2 (bản v1 thành phụ lục — chú thích mã trỏ "mục 7.2", "A1" là của phụ lục). Thư viện tri thức sự nghiệp `lib/rag/thu-vien/` + 1.091 mục trong `noi_dung_ai` bề mặt `thu-vien` (câu trích sách CHỈ ở Supabase). Lát cắt lượt 1 trượt so mù (~47% < 60%) → `CAU_THU_VIEN` trong route để RỖNG. Đo lại: `scripts/do-thu-vien.ts --ra <ngoài repo>` rồi `scripts/so-sanh-v3.ts A.json B.json --du`. Chờ chủ dự án chọn hướng (mục 11.3) | `314e6f3` | 26/09/2026 |
| **ĐÃ LÊN PRODUCTION**: CEL-140 mức tin cậy có tác dụng thật (truy hồi v3 2026.09.6 cộng điểm sau cổng khớp; nhãn [tin cậy] trong prompt 2026.09.17; kiểm `chi-bo-tro`) + admin gán lại mức tin cậy / loại nguồn ở `/admin/knowledge` (PATCH). CEL-141 "Ghi chú chuyên gia" / "Nội bộ" = luật ngầm (nhãn [LUẬT NGẦM], kiểm `lo-luat-ngam`); ghép nghĩa hai sao được phép trừ khi nguồn nói ngược, ý ghép gắn `ghep`. CEL-142 nút "Tạo bản mới" theo `phienBanKho()` (`lib/rag/tai-lieu-meta.ts`) — MỌI thay đổi kho bật nút cho mọi lá số, gom thay đổi kho thành đợt; bài cũ không dấu = kho `40749bb09806` | `35e9fd5` | 26/09/2026 |
| **ĐÃ LÊN PRODUCTION**: CEL-138 khoá đệm luận giải v3 băm theo KHUNG giờ sinh (`veGioLaSo` trong `lib/rag/nhat-ky.ts`, đọc lùi khoá giờ thô) — "load lại khi đổi thiết bị" thật ra do các lần tăng `THE_HE_DEM` (lá số chủ dự án sinh lại 8 lần 24–25/09): ĐỪNG tăng khi chưa hỏi chủ dự án. CEL-139 khách chưa đăng nhập tối đa 3 lá số mới/ngày/IP (`lib/auth/gioi-han-khach.ts`, đếm trong `noi_dung_ai` bề mặt `gioi-han-khach`, IP băm HMAC; env `GUEST_NEW_CHARTS_DAILY_LIMIT`) | `b29f1f1` | 26/09/2026 |
