...
as $fn$
  select
    c.id, c.version_id, c.document_id, c.noi_dung, c.duong_de_muc,
    d.tieu_de, d.he_phai, d.muc_tin_cay, v.phien_ban,
    ts_rank(to_tsvector('simple', c.noi_dung), plainto_tsquery('simple', cau_truy_van))::float as diem
  from public.knowledge_chunks c
  join public.knowledge_document_versions v on v.id = c.version_id
  join public.knowledge_documents d on d.id = c.document_id
  where v.trang_thai = 'da_xuat_ban'
    ...
    and to_tsvector('simple', c.noi_dung) @@ plainto_tsquery('simple', cau_truy_van)
  order by diem desc
  limit so_luong;
$fn$;
