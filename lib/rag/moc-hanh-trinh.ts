...
NĂM LUẬT CỨNG:
- CẢ DÃY KHÔNG ĐƯỢC CÙNG MỘT KHUÔN CÂU. Đọc dọc xuống mà thấy mọi dòng mở giống nhau là hỏng.
...
${bang}`;
  ...
  for (const m of mang) {
    ...
    if (/(?:hãy|bạn nên|cần phải|nên dành|đừng quên)/i.test(cau)) continue;
